...
- 每个平台包含服务、登录工具、人工恢复工具，共 9 个可执行文件。
...
# 可用 --go /absolute/path/to/go 指定 Go；默认输出 dist/v1.0.0。
```
...
确认 tag、附件和正文后可使用 GitHub CLI 创建草稿 Release，检查附件后再发布。不要覆盖已经发布的 `v1.0.0`：后续修复使用新的版本号。
