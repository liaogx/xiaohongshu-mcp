...
| 删除主动发表的评论 | `sent_comments` | 根据已知笔记查找本账号的评论 ID，打开“删除评论”确认框，核验请求同时匹配笔记 ID 和评论 ID | 网页没有账号全部已发评论列表；本地记录或给定笔记范围不等于全部历史 |
...
| 删除收到的评论 | `received_comments` | 删除自己笔记下收到的评论，执行前核验笔记所有权 | 别人在其他笔记下回复自己，原评论仍归别人所有；网页通知删除入口未实现，不能删除别人笔记下的他人评论 |
...
账号所有者明确授权后执行。一次最多处理一个候选，实际变更至少间隔 60 秒；过早调用返回 `waiting_interval` 和 `next_allowed_at`，不阻塞等待也不发起新的变更。同一账号的本地回执跨计划去重，`unknown` 或 `rejected` 会阻止继续自动执行。
