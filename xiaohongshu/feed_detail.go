...
func checkPageAccessible(page *rod.Page) error {
	...
	// Detail, comment and like actions share this gate. A missing error banner
	// does not prove access: the security QR page has no note error wrapper.
	probe, probeErr := readSearchPageProbe(page.Timeout(2 * time.Second))
	if probeErr != nil {
		return fmt.Errorf("PAGE_STATUS_UNCONFIRMED: 无法确认详情页访问状态: %w", probeErr)
	}
	if err := probe.accessError(); err != nil {
		return err
	}
	if !probe.HasInitialState {
		return fmt.Errorf("PAGE_DATA_UNAVAILABLE: 详情页没有初始数据（path=%s, readyState=%s）；不能当作已登录或可正常互动", probe.PagePath, probe.ReadyState)
	}
