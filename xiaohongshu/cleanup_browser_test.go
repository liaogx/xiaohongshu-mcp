...
func TestCleanupBrowserFixtures(t *testing.T) {
	...
	t.Run("cleanup note readiness requires a non guest account", func(t *testing.T) {
		p := b.NewPage()
		defer p.Close()
		require.NoError(t, p.SetDocumentContent(`<script>window.__INITIAL_STATE__={user:{userInfo:{value:{guest:true,userId:'fixture-guest'}}},note:{noteDetailMap:{'fixture-note':{note:{noteId:'fixture-note',interactInfo:{liked:true,collected:false}}}}}}</script>`))
		_, err := waitNoteReady(p, "fixture-note", false, 100*time.Millisecond, true)
		require.ErrorContains(t, err, "LOGIN_STATUS_UNCONFIRMED", "public note data alone is not cleanup authority")
		_, err = p.Eval(`()=>window.__INITIAL_STATE__.user.userInfo.value={guest:false,userId:'fixture-account'}`)
		require.NoError(t, err)
		state, err := waitNoteReady(p, "fixture-note", false, time.Second, true)
		require.NoError(t, err)
		require.Equal(t, "fixture-account", state.UserID)
