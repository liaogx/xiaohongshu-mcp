...
func (c *localCookie) write(cks []byte, seed int) error {
	...
	// Write a complete replacement before renaming so interruption cannot leave
	// a truncated session. The temporary and final credential files are 0600.
	f, err := os.CreateTemp(filepath.Dir(c.path), ".xhs-session-*")
	if err != nil {
		return errors.Wrap(err, "create private session file failed")
	}
	tmp := f.Name()
	defer os.Remove(tmp)
	defer f.Close()
	...
	if err := f.Sync(); err != nil {
		return errors.Wrap(err, "sync session file failed")
	}
	if err := f.Close(); err != nil {
		return errors.Wrap(err, "close session file failed")
	}
