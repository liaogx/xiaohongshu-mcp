...
func TestSaveCookies_PrivateAtomicReplacement(t *testing.T) {
	path := filepath.Join(t.TempDir(), "cookies.json")
	assert.NoError(t, os.WriteFile(path, []byte(`[]`), 0644))
	c := NewLoadCookie(path)
	assert.NoError(t, c.SaveCookies([]byte(`[{"name":"fixture","value":"synthetic"}]`)))
	info, err := os.Stat(path)
	assert.NoError(t, err)
	assert.Equal(t, os.FileMode(0600), info.Mode().Perm())
	before, err := os.ReadFile(path)
	assert.NoError(t, err)
	assert.Error(t, c.SaveCookies([]byte(`invalid JSON`)))
	after, err := os.ReadFile(path)
	assert.NoError(t, err)
	assert.Equal(t, before, after, "failed serialization must leave old session intact")
	entries, err := os.ReadDir(filepath.Dir(path))
	assert.NoError(t, err)
	assert.Len(t, entries, 1, "no temporary credential files left behind")
}
